...
stage1<-function(){
  ...
  fail<-function(){
    # head出边界?
    if(length(which(e$head<1))>0 | length(which(e$head>e$width))>0){  # ????ͷ???????»?????
      print("game over: Out of ledge.")
      keydown('q')
      play("C:/Users/ing4/Documents/yyy/error.mp3")
      return(TRUE)
    }
  ...
  body<-function(){
    ...
    if(length(index(e$col_furit))<=0){ # 如果蛇吃掉水果后
      ...
      play("C:/Users/ing4/Documents/yyy/Score.mp3")
    }
...
stage0<-function(){
  ...
  text(0.2,0.05,label="Author:DanZhang(fyy)",cex=1)
...
stage3<-function(){
  e$stage<-3
  plot(0,0,xlim=c(0,1),ylim=c(0,1),type='n',xaxs="i", yaxs="i")
  text(0.5,0.7,label="Time out",cex=5)
